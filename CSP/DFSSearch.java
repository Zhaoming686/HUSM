...
public class DFSSearch
{
	...
	public DFSSearch(Graph g, ConstraintGraph cg, int minFreqThreshold, HashMap<Integer, HashSet<Integer>> nonCands)
	{
		//新加的
		this.graph = g;
	...
	public double getUpperBound(String Type){
		if (Type.equals("GWU")) return GWU;
		if (Type.equals("RMU")) return RMU;
		return -1;
	}
	...
	public void searchExistances()
	{
		...
		//to record each GWU, RMU and utility for each f_U0
		Map<Integer, Double> GWUTemp = new HashMap<>();
		Map<Integer, Double> RMUTemp = new HashMap<>();
		Map<Integer, Double> UtilityTemp = new HashMap<>();
		...
		for (int i = variables.length-1; i >=0 ; i--) 
		{
			...
			for (Iterator<myNode> iterator = firstList.values().iterator(); iterator.hasNext();)
			{
	        	...
	        	//新加的
	        	if (value==-1) {//found
					int valueCompute = searchVerticesCover(instance);
					if (valueCompute != -1) {//valid mapping
//						int f_U0 = instance.getAssignment(0).getID();
//						if (!GWUTemp.containsKey(f_U0)){
//							Settings.Count++;
//							Settings.list.add(f_U0);
//							GWUTemp.put(f_U0,valueCompute.getA());
//							RMUTemp.put(f_U0,valueCompute.getB());
//							UtilityTemp.put(f_U0,valueCompute.getC());
//						}else {
//							if (GWUTemp.get(f_U0) < valueCompute.getA()){
//								GWUTemp.put(f_U0,valueCompute.getA());}
//							if (RMUTemp.get(f_U0) < valueCompute.getB()){
//								RMUTemp.put(f_U0,valueCompute.getB());}
//							if (UtilityTemp.get(f_U0) < valueCompute.getC()){
//								UtilityTemp.put(f_U0,valueCompute.getC());}
//						}
					}else{
						value = -2;
				...
				if(value==-1)
				{
					...
					if (!result[index].getList().isEmpty() && index!=0){
						double UtilityList = 0;
						for (myNode u : result[index].getList().values()){
							UtilityList += u.getWeight();
						}
						if (UtilityList>Settings.utilityThre) break;
					}
				}
				else if(value>=0)
					if(Settings.PRINT)
...
//								break;
			...
			if (index==0){
				if (!result[0].getList().isEmpty()){
					double GWUList = 0;
					for (myNode u : result[0].getList().values()){
						GWUList += searchMSS(u.getID(),"GWU");
					}
					if (GWUList < Settings.utilityThre) return;
				}
			}
		}
		...
		// apply the value to GWU RMU and utility
//		if (!GWUTemp.isEmpty()){
//			for (double u : GWUTemp.values()) GWU += u;
//			for (double u : RMUTemp.values()) RMU += u;
//			for (double u : UtilityTemp.values()) utility += u;
//		}
//		System.out.println("the GWU is: "+ GWU);
//		System.out.println("the RMU is: "+ RMU);
//		System.out.println("the Utility is: "+ utility);
	}

	/***********新加************/
	//新加的计算weight的一些东西
	private double GWU = 0, RMU = 0, utility = 0;
	private Graph graph;

	public double searchMSS(int sourceMap, String Type){
		//找符合要求的点，即把该起点的所有的图勾勒出来
		DenseRoutesMap map = new DenseRoutesMap(this.graph);
		int MVC = Settings.MVC;//这个记得改成setting的东西
		DijkstraEngine dj = new DijkstraEngine(map, MVC);
		dj.execute(sourceMap, null);
		List<Integer> verticesIDsCovered = dj.getVerticesIDsCovered();
		//计算GWU, RMU and Utility of each mapping
		if (Type.equals("GWU")){
			Double GWUEachMapping = ComputeGWUEachMapping(verticesIDsCovered);
			return GWUEachMapping;
		}
		if (Type.equals("RMU")){
//			Double RMUEachMapping = ComputeRMUEachMapping(verticesIDsCovered, sourceMap);
//			return RMUEachMapping;
			ComputeRMUEachMapping(verticesIDsCovered, sourceMap);
			return -1;
		}
		return 0;
	}

	public void SetUpperBoundOfPattern()
	{
		...
		if (min == 0) return;
		//compute GWU
		List<Double> GwuList = new ArrayList<>();
		for (myNode U0 : result[0].getList().values()){
			double GwuTemp = searchMSS(U0.getID(), "GWU");
			GwuList.add(GwuTemp);
		}
		Collections.sort(GwuList);
		for (int i = GwuList.size()-1; i >= GwuList.size()-min; i--){
           this.GWU += GwuList.get(i);
		}
		//compute RMU
		List<List<Double>> RmuList = new ArrayList<>();
		for (myNode U0 : result[0].getList().values()){
			searchMSS(U0.getID(), "RMU");//构建那个成员变量
		}
		double RMUtemp = 0;
		for (int label : LabelWeight.keySet()){
			List<Double> weights = LabelWeight.get(label);
			Collections.sort(weights);
			//计算值
			double valueTemp = 0;
			if (weights.size()>=min) {
				for (int i = weights.size() - 1; i >= weights.size() - min; i--) {
					valueTemp += weights.get(i);
				}
			}else{
			...
			//找最大值
			if (valueTemp > RMUtemp) RMUtemp = valueTemp;
		}
		this.RMU = RMUtemp;
        if (RMU > GWU){
			Settings.Count++;
        ...
        //compute RMU
//		List<Double> RmuList = new ArrayList<>();
//		for (myNode U0 : result[0].getList().values()){
//			double RmuTemp = searchMSS(U0.getID(), "RMU");
//			RmuList.add(RmuTemp);
//		}
//		Collections.sort(RmuList);
//		for (int i = RmuList.size()-1; i >= RmuList.size()-min; i--){
//			this.RMU += RmuList.get(i);
//		}
		// computeUtility
		List<Double> ResultTemp = new ArrayList<>();
		List<Double> UtilityTemp = new ArrayList<>();
		for (int i = 0; i < result.length; i++){
			for (myNode u : result[i].getList().values()){
				ResultTemp.add(u.getWeight());
			}
			Collections.sort(ResultTemp);
			double MaxUtilityTemp = 0;
//			for (int j = ResultTemp.size()-1; j >= ResultTemp.size()-min; j--){
			for (int j = 0; j < min; j++){
				MaxUtilityTemp += ResultTemp.get(j);
			}
			UtilityTemp.add(MaxUtilityTemp);
		}
		Collections.sort(UtilityTemp);
		this.utility = UtilityTemp.get(UtilityTemp.size()-1);
	...
	public int searchVerticesCover(AssignmentInstance instance){
		//找符合要求的点，即把该起点的所有的图勾勒出来
		int source = instance.getAssignment(0).getID();
		DenseRoutesMap map = new DenseRoutesMap(this.graph);
		int MVC = Settings.MVC;//这个记得改成setting的东西
		DijkstraEngine dj = new DijkstraEngine(map, MVC);
		dj.execute(source, null);
		List<Integer> verticesIDsCovered = dj.getVerticesIDsCovered();
		//判断instance是否有效。
		if (!isMVC(verticesIDsCovered, instance)) return -1;
		//计算GWU, RMU and Utility of each mapping
//		Double GWUEachMapping = ComputeGWUEachMapping(verticesIDsCovered);
//		Double RMUEachMapping = GWUEachMapping;
//		Double UtilityEachMapping = ComputeUtilityEachMapping(instance);
//		//记录并返回（在dataStructures里面加了个三元组的类）
//		ValueCompute<Double, Double, Double> valueCompute = new ValueCompute<>(GWUEachMapping, RMUEachMapping, UtilityEachMapping);
		return 1;
	}
	/**
	 * 辅助函数1，判断这个instance是否符合要求，即是否在范围里面，若否则返回null
	 */
	public Boolean isMVC(List<Integer> verticesIDsCovered, AssignmentInstance instance){
		for (int i = 0; i < instance.getAssignmentSize(); i++){
			int idMap = instance.getAssignment(i).getID();
			if (!verticesIDsCovered.contains(idMap)) return false;
		}
		return true;
	}

	public Double ComputeGWUEachMapping(List<Integer> verticesIDsCovered){
		double MaxValueInMSS = 0;
		for (int MappedId : verticesIDsCovered){
			if (this.graph.getNode(MappedId).getWeight() > MaxValueInMSS){
				MaxValueInMSS = this.graph.getNode(MappedId).getWeight();
			}
		}
		return MaxValueInMSS;
	}

	private Map<Integer, List<Double>> LabelWeight = new HashMap<>();

	public void ComputeRMUEachMapping(List<Integer> verticesIDsCovered, int sourceMap){
		Map<Integer, Double> LabelWeightMSS = new HashMap<>();
		for (int MappedId : verticesIDsCovered){
			int Sourcelabel = this.graph.getNode(sourceMap).getLabel().get(0);
			int SourceOrder = this.graph.getSortedFreqLabels().indexOf(Sourcelabel);
			int Mappedlabel = this.graph.getNode(MappedId).getLabel().get(0);
			int MappedOrder = this.graph.getSortedFreqLabels().indexOf(Mappedlabel);
			if (MappedOrder >= SourceOrder){
				if (LabelWeightMSS.containsKey(Mappedlabel)){
					if (this.graph.getNode(MappedId).getWeight() > LabelWeightMSS.get(Mappedlabel)){
						LabelWeightMSS.put(Mappedlabel, this.graph.getNode(MappedId).getWeight());
					}
				}else{
					LabelWeightMSS.put(Mappedlabel, this.graph.getNode(MappedId).getWeight());
				}
		...
		for (int label : LabelWeightMSS.keySet()){
			if (LabelWeight.containsKey(label)){
				LabelWeight.get(label).add(LabelWeightMSS.get(label));
			}else{
				List<Double> weights = new ArrayList<>();
				weights.add(LabelWeightMSS.get(label));
				LabelWeight.put(label, weights);
			}
		}
	}

//	public Double ComputeRMUEachMapping(List<Integer> verticesIDsCovered, int sourceMap){
//		double MaxValueInMSS = 0;
//		for (int MappedId : verticesIDsCovered){
//            int Sourcelabel = this.graph.getNode(sourceMap).getLabel().get(0);
//			int SourceOrder = this.graph.getSortedFreqLabels().indexOf(Sourcelabel);
//			int Mappedlabel = this.graph.getNode(MappedId).getLabel().get(0);
//			int MappedOrder = this.graph.getSortedFreqLabels().indexOf(Mappedlabel);
//			if (this.graph.getNode(MappedId).getWeight() > MaxValueInMSS && MappedOrder>=SourceOrder){
//				MaxValueInMSS = this.graph.getNode(MappedId).getWeight();
//			}
//		}
//		return MaxValueInMSS;
//	}
	...
	public Double ComputeUtilityEachMapping(AssignmentInstance instance){
		double MaxValueInOcc = 0;
		for (int i = 0; i < instance.getAssignmentSize(); i++){
			int MappedId = instance.getAssignment(i).getID();
			if (this.graph.getNode(MappedId).getWeight() > MaxValueInOcc){
				MaxValueInOcc = this.graph.getNode(MappedId).getWeight();
			}
		}
		return MaxValueInOcc;
	}
