	...
	{		
		...
		if(Settings.multipleAtts)
		{
			...
			while ((line = rows.readLine()) !=null && (line.charAt(0) == 'v')) {
				...
				if(isNewNode)
				{
					...
					nLabels.add(label);
					n = new myNode(numberOfNodes, nLabels, weight);
					nodes.add(n);
					numberOfNodes++;
				}
				else
		...
		else
		{
			...
			while ((line = rows.readLine()) !=null && (line.charAt(0) == 'v')) {
				...
				//新加的weight
				final double weight = Double.parseDouble(parts[3]);
				//final int weight = Integer.parseInt(parts[3]);
				counter++;
				...
				ArrayList<Integer> nLabels= new ArrayList<Integer>();
				nLabels.add(label);
				myNode n = new myNode(numberOfNodes, nLabels, weight);
				nodes.add(n);
				HashMap<Integer,myNode> tmp = nodesByLabel.get(label);
				if(tmp==null)
				{
		...
		if(line!=null)
		{
			do
			{
				...
				final double label = Math.round(Double.parseDouble(parts[3]));
